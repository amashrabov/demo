...
class DeployBuilder:
    def __init__(self, app_config: AppConfig, project_path: Path):
        ...
        self.keys = list(dotenv.dotenv_values(self.project_path / "env").keys())
        self.keys.pop(self.keys.index("SSH_KEY"))
        self.echo_indent = "          "
    ...
    def generate(self):
        ...
        if keyed_repo_url is None:
            raise ValueError(
                "Are you sure you have add git remote origin? / pushed to github?"
            )
        ...
        path.write_text(
            template.render(
                ...
                env_gen=insert_env_line(self.keys, self.echo_indent),
            )
        )
